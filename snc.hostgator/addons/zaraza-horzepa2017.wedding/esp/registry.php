<?php 
	include($_SERVER['DOCUMENT_ROOT'].'/php/autoVer.php');
?>
<!DOCTYPE html>
<html class="no-js" lang="en-US">
<head>
	<meta charset="utf-8">
	<meta http-equiv="x-ua-compatible" content="ie=edge">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Wedding Registry | Zaraza-Horzepa Wedding</title>
	<meta name="description" content="">
	<link href="http://zaraza-horzepa2017.wedding/registry" rel="canonical">
	<link href="/humans.txt" rel="author">

	<link href="<?php autoVer('/css/app.css'); ?>" rel="stylesheet">
		
</head>
<body id="registry" class="espanol">



<div class="off-canvas position-right" id="offCanvas" data-transition="overlap" data-off-canvas>
	<!-- Close button -->
	<button class="close-button" aria-label="Close menu" type="button" data-close>
		<span aria-hidden="true">&times;</span>
	</button>

	<!-- Menu -->
	<ul class="vertical menu">
		<li><a href="/esp">Nuestra Historia</a></li>
		<li><a href="http://www.rsvp.zaraza-horzepa2017.wedding/mx/tus-invitados/request-rsvp-by-guest?_ga=1.201991125.514128241.1485290807" target="_blank">RSVP</a></li>
		<li><a href="/esp/wedding-details">Detalles de La Boda</a></li>
		<li><a href="/esp/cartagena-info">Información de Cartagena</a></li>
		<li class="active"><a href="/esp/registry">Lista de Boda</a></li>
	</ul>
</div>
<div class="off-canvas-content" data-off-canvas-content style="background: none;">
	<header>
		<div class="row">
		<button class="float-right menu-icon hide-for-large" type="button" data-open="offCanvas"></button>
			<ul class="menu languageSwitches float-left hide-for-large">
				<li><a href="/registry"><img srcset="/img/icons/eng_2x.png 2x" src="/img/icons/eng.png" alt="English"></a></li>
				<li><a href="/esp/registry"><img srcset="/img/icons/esp_2x.png 2x" src="/img/icons/esp.png" alt="Spanish"></a></li>
			</ul>
			<div class="small-12 columns">
				<ul class="menu languageSwitches float-left show-for-large">
				<li><a href="/registry"><img srcset="/img/icons/eng_2x.png 2x" src="/img/icons/eng.png" alt="English"></a></li>
				<li><a href="/esp/registry"><img srcset="/img/icons/esp_2x.png 2x" src="/img/icons/esp.png" alt="Spanish"></a></li>
				</ul>
				<nav class="mainNav text-right medium-text-left show-for-large">
					<ul class="menu float-right">
						<li><a style="width: 160px;" href="/esp">Nuestra Historia</a></li>
						<li><a href="http://www.rsvp.zaraza-horzepa2017.wedding/mx/tus-invitados/request-rsvp-by-guest?_ga=1.201991125.514128241.1485290807" target="_blank">RSVP</a></li>
						<li><a style="width: 160px;" href="/esp/wedding-details">Detalles de La Boda</a></li>
						<li><a style="width: 210px;" href="/esp/cartagena-info">Información de Cartagena</a></li>
						<li class="active"><a style="width: 130px;" href="/esp/registry">Lista de Boda</a></li>
					</ul>
				</nav>
			</div>
		</div>
	</header>
	<div class="row">
		<div class="medium-6 medium-push-6 columns">
			<aside class="pageTitle">
				<h1 class="script">Wedding Registry</h1>
			</aside>
		</div>
	</div>
	<div class="row">
		<article class="small-10 small-push-1 columns">
			<h3 class="text-center">La presencia de ustedes en nuestra boda es el mejor <span class="inline-block">regalo del mundo.</span></h3>
			<div class="row">
				<div class="medium-4 columns text-center">
					<img src="/img/registry/antonella.jpg" alt="Antonella">
				</div>
				<div class="medium-8 columns">
					<p>Sin embargo, sí desean honrarnos con un regalo, por favor ayúdenos a encontrar la cura contra el síndrome de Angelman. Las donaciones las pueden hacer a <a href="http://cureangelman.org/donate/in-honor-of/" target="_blank">www.cureangelman.org/donate/in-honor-of/</a>. Te pedimos hacer la donación a nombre a <span class="inline-block">Antonella Grace Woodall.</span></p>
					<h5>Mil gracias, Juliana &amp; Joe</h5>
				</div>
			</div>
		</article>
	</div>
</div>



<script src="https://ajax.googleapis.com/ajax/libs/jquery/2.2.4/jquery.min.js"></script>
<script>window.jQuery || document.write('<script src="/js/vendor/jquery/2-2-4/jquery-min.js"><\/script>')</script>
<script src="<?php autoVer('/js/app-min.js'); ?>"></script>
</body>
</html>